...
(`Proto_Elamite_writing_in_Iran.pdf`): 17 files under `francois_desset/` ("more by
this author" — verified: genuinely all François Desset, solo or co-authored, no author-mismatch
found this time) and 20 files under `origins_of_writing/` (mixed authors/topics). Every
...
- `origins_of_writing/Breaking_the_Code_Ancient_Irans_Linear_E.pdf` — byte-identical
  (same file size, 398307 bytes) to `PDFs/Breaking_the_Code_Ancient_Irans_Linear_E.pdf`, already indexed above.
- `origins_of_writing/A_NEW_WRITING_SYSTEM_DISCOVERED_IN_3rd_M.pdf` — same Desset 2014
...
- `francois_desset/Linear_Elamite_writing.pdf` — same "Linear Elamite Writing" chapter
...
- `francois_desset/Petrie_C_A_ed_2013_Ancient_Iran_and_Its.pdf` — front-matter-only
...
`francois_desset/` — all are legitimate Desset (co-)authored publications, but about
...
`origins_of_writing/` — legitimate scholarship, but about other regions/scripts (Levant,
