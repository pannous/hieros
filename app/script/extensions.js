...
if (typeof(document) == 'undefined'){
    ...
    document = {currentScript: {hasAttribute: x => 1, getAttribute: x => "program.wasm"}}
}
else context=Context.web

try {
    if (typeof(XMLHttpRequest) == 'undefined')
        XMLHttpRequest = require("xmlhttprequest").XMLHttpRequest;
} catch (ex) {
console.error("No XMLHttpRequest -> node install xmlhttprequest")// ex)
}
...
try {
    ...
    stringify = serialize = serial = x => isString(x)?x:JSON5.stringify(x)
    deserialize = content => {
        try {
    ...
    json = JSON.parse
    stringify = serialize = serial = x => isString(x)?x:JSON.stringify(x)
...
is_file = isFile = test => {
    try {
        return fs.lstatSync(test).isFile() || is_string(test) && test.match(/\.\w+$/)
    } catch (ex) {
        return false
    }
}
...
string = x => x.toString()
// decodeURL = decodeUriComponent = require('decode-uri-component')
// const { StringDecoder } = require('string_decoder');
// const decoder = new StringDecoder('utf8');
try {
    TextEncoder = require('text-encoding').TextEncoder // node.js
    TextDecoder = require('text-encoding').TextDecoder // node.js
    decoder = new TextDecoder('utf-8')
    encoder = new TextEncoder('utf-8')
    // ascii = new TextDecoder('ascii') not much faster
} catch (x) {
}
...
Map_Extensions = {
// > m=new Map([['a', 0], ['b', 1]]) // => Map { 'a' => 0, 'b' => 1 }
// > m.a || m['a'] 
// undefined // WTF !!!
// > m.get('a')
    toObject(){
        let obj = Object.create(null);
        for (let [k,v] of this) obj[k] = v;
        return obj;
    }
}
...
ArrayBuffer_Extensions = {
    copy(dst)  {
        if(!dst)dst = new ArrayBuffer(this.byteLength);
    ...
    set(data,position){
        // let view=new Uint8Array(this.slice(position,data.length))// slicing is NOT A VIEW!!!
        let view=new Uint8Array(this , position, data.length)
        if(!is_array(data))data=[data]
        view.set(data,0)
    }
}

Uint8Array_Extensions = {
...
Array_Extensions = {
    ...
    append(x) {
        if (isList(x)) this.merge(x)
        else this.push(x) // RETURNS X WTF!!!   this[this.len] = (x);
        return this
...
String_Extensions = { // StringExtensions
    at(i) {
        return unicode(this.codePointAt(i >= 0 ? i : this.length + i))
    },
    char(i) {
        return unicode(this.codePointAt(i >= 0 ? i : this.length + i))
    },
...
var all = all_extensions = function () { // needs to be assigned per context !?
    ...
    Object.assign(ArrayBuffer.prototype, ArrayBuffer_Extensions) // join / merge into!
    Object.assign(Function.prototype, Function_Extensions)
    Object.assign(Map.prototype, Map_Extensions)
    if (typeof(Buffer) != 'undefined')
...
if (context==Context.node) { try {
    ...
    miss=(pkg)=>console.log("Missing require: "+pkg)
    
    try {curl = require('request'); } catch (ex) {miss('request')}
    try {if(typeof(window)!='undefined')window.WebSocket = require('websocket').w3cwebsocket } catch (ex) {miss('websocket')}
    ...
    w = wb = write = save = dump = dumb = writeFile = (data, file = 'data.bin') =>
        isFile(data) ? fs.writeFileSync(data, serialize(file), 'utf8') : fs.writeFileSync(file, serialize(data), 'utf8')
    ap = append = (text,file) => fs.appendFileSync(file, text + "\n") // , err=>{p(err)}
    dump_json5 = write_json5 = save_json5 = (file, obj) => save(file, JSON5.stringify(obj));
    load_json5 = read_json5 = (file) => JSON5.parse(read(file));
    parse_json5 = (js5) => JSON5.parse(js5);
    json5 = (js5) => isString(js5)?JSON5.parse(js5):JSON5.stringify(js5);
    read = read_raw = read_file = fs.readFileSync
    read_text=(path)=>fs.readFileSync(my(path)).toString()
    ...
    util = require('util') // NODE.JS extensions
    let {lstatSync, readdirSync} = require('fs')
    isDir = x => { try {
            return x && lstatSync(x).isDirectory()
        } catch (ex) {
            console.error(ex.message);
            return false;
    }  }
    // ls = readdirSync
    ls=files=list_files=x=>run('ls '+(x||''))  // dir is python: show properties!!
...
class FakePromise {
    constructor(data) {
        this.data = data
        this.data.arrayBuffer = x => this
        // this.data.text=x=>this.data
        // this.data.json=x=>this.data
    }
...
if (typeof(fetch) == 'undefined') {
    try {
        fetch = file => {
            return file.match("//") ? require('node-fetch')(file) : new FakePromise(load_binary(file))
            // new Promise(function(resolve, reject) {
            // 	resolve(load_binary(file))
            // })
        }
    } catch (ex) {
        console.log(ex)
...
missing_dependency =(module,fun)=>function (x,y,z,a) {
    console.log("missing dependency "+module+"." +fun + "(", x,y,z,a,")")
}
...
wasm = async (_wasm, imports = {}) => {
    ...
    if (is_string(_wasm) && !_wasm.contains('.')) _wasm += ".wasm"
    if (is_file(_wasm)) {
    ...
    if (!Object.keys(imports).length) imports = {
        ...
        console: {},
        env: {
            logc, _logc: logc, logs, _logs: logs, logi: log, _logi: log, log, _log: log,raise,logx,
            memory, table, abort: nop, nullFunc_X: log, abortStackOverflow: log, // (x => {throw "stack overflow:" + x }),
    ...
    try {
        let module = await WebAssembly.compile(_wasm)// global
    ...
    new_string = (s) => {
        ...
        for (let i = 0; (i + offset) < buffer.length && i < arg.length; i++) {
            if (buffer[i + offset]) {
                console.log("wasm memory not empty");
                break
            }
...
function switch_keys_and_values(map) {
    ...
    if (is_array(map))
        for (i in map) {
            if (map.hasOwnProperty(i)) res[map[i]] = i
        }
    else
...
flip = invert = switch_keys_and_values // can't attach to map, cause Object
