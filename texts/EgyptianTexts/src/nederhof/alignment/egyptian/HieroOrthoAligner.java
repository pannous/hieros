...
public abstract class HieroOrthoAligner {
    ...
    private static void getNamesOrtho(Tier tier, Vector<String> names, 
	    Vector<Integer> namePositions) {
	TierPart[] parts = tier.parts;
	int nSymbols = 0;
	for (int i = 0; i < parts.length; i++) {
	    TierPart part = parts[i];
	    Vector glyphs = glyphsOfOrtho(part);
	    if (glyphs != null)
		for (int j = 0; j < glyphs.size(); j++) {
		    ResNamedglyph named = (ResNamedglyph) glyphs.get(j);
		    names.add(hieroContext.nameToGardiner(named.name));
    ...
    private static Vector glyphsOfOrtho(TierPart part) {
        if (part instanceof OrthoPart) {
            OrthoPart ortho = (OrthoPart) part;
            if (ortho.texthiParsed != null)
                return ortho.texthiParsed.glyphs();
        } else if (part instanceof OrthoPdfPart) {
            OrthoPdfPart ortho = (OrthoPdfPart) part;
            if (ortho.texthiParsed != null)
                return ortho.texthiParsed.glyphs();
        }
        return null;
    ...
    public HieroOrthoAligner(String[] glyphs1, String[] glyphs2) {
