...
public class TreeHorgroup extends ResHorgroup
		implements TreeTopgroup, TreeVertsubgroupPart {

	// Constructor with only two groups.
	public TreeHorgroup(TreeHorsubgroupPart group1,
			TreeHorsubgroupPart group2) {
	...
	public TreeHorgroup(TreeHorsubgroupPart group1,
			...
			TreeHorsubgroupPart group2) {
		super(new TreeHorsubgroup(group1), 
				op, switchs, 
				new TreeHorsubgroup(group2));
		panel = new NodePanel(this);
		op.changed = true;
	}
	// Auxiliary to constructor.
	private void addTreeGroup(TreeOp op, TreeSwitch switchs,
			TreeHorsubgroupPart group) {
		addGroup(op, switchs, new TreeHorsubgroup(group));
		op.changed = true;
	}

	// Constructor.
	public TreeHorgroup(ResHorgroup group) {
		super(TreeHorsubgroup.makeGroups(group.groups),
	...
	public LegendStructure makeStructureButtons() {
		...
		buttons.addButtonLeft(
				...
				protected void pushed() {
					prepareStructChange();
					EditHelper.appendHor(tGroup(nGroups()-1));
					finishStructChange();
				} });
		buttons.addButtonLeft(
				new StructureButton("+", '+') {
				protected void pushed() {
					prepareStructChange();
					EditHelper.prependHor(tGroup(0));
					finishStructChange();
				} });
		buttons.addButtonLeft(
				new StructureButton(":", ':') {
				protected void pushed() {
					prepareStructChange();
					EditHelper.appendVert(TreeHorgroup.this);
					finishStructChange();
				} });
		buttons.addButtonLeft(
				new StructureButton(";", ';') {
				protected void pushed() {
					prepareStructChange();
					EditHelper.prependVert(TreeHorgroup.this);
					finishStructChange();
				} });
		buttons.addButtonLeft(
				new StructureButton("-", '-') {
				protected void pushed() {
					prepareStructChange();
					EditHelper.appendNamedBehind(TreeHorgroup.this);
					finishStructChange();
				} });
		buttons.addButtonRight(
				...
				protected void pushed() {
					prepareStructChange();
					EditHelper.placeInBox(TreeHorgroup.this);
					finishStructChange();
				} });
		buttons.addButtonRight(
				...
				protected void pushed() {
					prepareStructChange();
					EditHelper.placeInStack(TreeHorgroup.this);
					finishStructChange();
				} });
		buttons.addButtonRight(
				...
				protected void pushed() {
					prepareStructChange();
					EditHelper.placeInInsert(TreeHorgroup.this);
					finishStructChange();
				} });
		buttons.addButtonRight(
				...
				protected void pushed() {
					prepareStructChange();
					EditHelper.placeInModify(TreeHorgroup.this);
					finishStructChange();
				} });
	...
	public void insertAt(TreeHorsubgroupPart newGroup, int i) {
		addGroupAt(new TreeHorsubgroup(newGroup), 
